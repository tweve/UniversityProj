...
void invertRow(int pixelNo, char* row)
{
	pixel aux;
	int i;
	
	//for each pixel
	for(i=0; i < (pixelNo*3)/2; i=i+3)
	{
		...
		row[i] = row[3*pixelNo-i-3];
		row[i+1] = row[3*pixelNo-i-2];
		row[i+2] = row[3*pixelNo-i-1];
		
		
		row[3*pixelNo-i-3] = aux.red;
		row[3*pixelNo-i-2] = aux.green;
		row[3*pixelNo-i-1] = aux.blue;
	}
}
