...
def loadFilesUntil(lastNumber):
    ...
    if lastNumber < 1:
        print "Arg should be bigger than 0"
        return None
    
    names = []
    for i in range(1, lastNumber+1):
        names.append("training_" + str(i) + ".iia")

    return loadFilesFromList(names)

def loadFilesFromList(filesNames):
...
def getActionRepresentation(output):
    
    from game import Directions
    if output == Directions.NORTH:
        return [1, 0, 0, 0, 0]
    if output == Directions.SOUTH:
        return [0, 1, 0, 0, 0]
    if output == Directions.EAST:
        return [0, 0, 1, 0, 0]
    if output == Directions.WEST:
        return [0, 0, 0, 1, 0]
    if output == Directions.STOP:
        return [0, 0, 0, 0, 1]
